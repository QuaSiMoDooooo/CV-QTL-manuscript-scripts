...
out_dir <- "results/epi_enrichment/chromHMM/stat_qtl"
    ...
    for (qtl_type in qtl_types) {
        ...
        for (n in 1:129) {
            ...
            if (file.exists(result_path)) {
                ...
                merged_cell_line_enrich[[paste0(variant_type, "-", qtl_type)]] <- merged_data
            }
        }
    }
}
...
temp <- sapply(
    ...
    FUN = function(name) {
        data <- merged_cell_line_enrich[[name]] %>%
            ...
            pivot_longer(
                cols = -c(annotation, variant_type, qtl_type),
                names_to = "cell_line",
                values_to = "stat"
            ) %>%
...
fwrite(
    x = plot_df_adj,
    file = file.path(out_dir, "enrich_chromHMM_qtl.txt"),
    sep = "\t",
    quote = FALSE
)

plot_by_both_all <- ggplot(
    data = plot_df_adj,
    mapping = aes(
        x = cell_line,
        y = variant_type,
...
plot_by_both_all_blood <- ggplot(
    data = plot_df_adj %>% filter(category == "Blood"),
    mapping = aes(
        x = cell_line,
        y = variant_type,
        fill = log2FC
...
ggsave(
    file = file.path(out_dir, "plot_by_both_all.pdf"),
    plot = plot_by_both_all,
    width = 10,
    height = 4
)
ggsave(
    file = file.path(out_dir, "plot_by_both_all_blood.pdf"),
    plot = plot_by_both_all_blood,
    width = 10,
    height = 8
)
