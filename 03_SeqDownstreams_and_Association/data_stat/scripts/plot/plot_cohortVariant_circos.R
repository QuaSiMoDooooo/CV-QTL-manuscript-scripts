...
# Last Modified: 2025-03-03 21:53
...
snp_density <- fread("results/stat/circos_density/SNP_density.txt")
indel_density <- fread("results/stat/circos_density/InDel_density.txt")
mnv_density <- fread("results/stat/circos_density/MNV_density.txt")
sv_density <- fread("results/stat/circos_density/SV_all_density.txt")
...
# tr_density <- fread("results/stat/circos_density/TR_density.txt")

pdf(
    "results/plot/cohortVariant_circos.pdf",
    width = 8,
    height = 8,
    onefile = TRUE
)
...
legend_variant <- Legend(
    labels = c("SNP", "InDel", "MNV", "SV"),
    type = "grid",
    border = NULL,
    background = NULL,
    legend_gp = gpar(col = custom_colors[1:4], fill = custom_colors[1:4]),
