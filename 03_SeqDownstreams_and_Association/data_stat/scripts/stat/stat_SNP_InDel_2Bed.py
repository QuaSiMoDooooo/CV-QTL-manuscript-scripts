...
SNP_cohort_path = "data/cohort_SNP_InDel.vcf.gz"
...
with gzip.open(SNP_cohort_path, "rt") as VCF:
    for line in VCF:
        ...
        # 忽略复等位基因变异
        if "," in line_splited[3] or "," in line_splited[4]:
            continue
        # 太长的indel忽略
        if len(line_splited[3]) > 50 or len(line_splited[4]) > 50:
            continue
        # SNP
        elif len(line_splited[3]) == 1 and len(line_splited[4]) == 1:
