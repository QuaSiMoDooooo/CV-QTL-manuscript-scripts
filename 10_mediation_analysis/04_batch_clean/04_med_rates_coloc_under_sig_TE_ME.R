...
# -------------
# FileName     : 04_med_rates_coloc_under_sig_TE_ME.R
# Author       : Tian Wu fabien012390wt@163.com
# Modified By  : Tian Wu
# -------------
# Description  : Calculate mediation rates under significant total and mediated effects of colocalization results
# -------------
...
coloc_dir = "/home/wtian/project/HZAU_cohort_meth/wdy_assist_rerun/11_coloc/05_coloc"
...
for (m in 1:length(med_type_list)){
    ...
    for (j in 1:length(variant_type_list)){
        ...
        med_res_all$exp = str_split(med_res_all$exp,"\.",simplify = T)[,1] # nolint: error.
        med_res_all$med_exp = paste0(med_res_all$med, "::", med_res_all$exp)

        if (grepl("GWAS", med_type)){
            coloc_file = paste0(coloc_dir,"/03_gwas_qtl_colocResultsig.tsv")
        }else if (grepl("cis_QTL", med_type)){
            coloc_file = paste0(coloc_dir,"/03_eqtl_qtl_colocResultsig.tsv")
        }
        coloc = fread(coloc_file)

        colnames(coloc)[1] = "exp"
        coloc$med_exp = paste0(coloc$phe, "::", coloc$exp)

        med_res_all = filter(med_res_all, med_exp %in% coloc$med_exp)
...
write.table(all_stats,"04_all_stats.tsv",quote = F,row.names = F,sep = "\t")
...
for (m in 1:length(med_type_list)){
    ...
    for (j in 1:length(variant_type_list)){
        ...
        med_res_all_path = paste0(med_type,"/",variant_type,"/med_res_all.tsv")
        med_res_all = fread(med_res_all_path)
        med_res_all$exp = str_split(med_res_all$exp,"\.",simplify = T)[,1]
        med_res_all$med_exp = paste0(med_res_all$med, "::", med_res_all$exp)
...
write.table(all_med_rates,"04_all_med_rates_under_sig_TE_ME.tsv",quote = F,row.names = F,sep = "\t")
...
write.table(all_med_rates_grouped,"04_all_med_rates_under_sig_TE_ME_grouped.tsv",quote = F,row.names = F,sep = "\t")

tmp = data.frame(
    type = paste0(str_split_fixed(all_med_rates$med_type,"_",2)[,2],":",all_med_rates$variant_type),
    med_rate = all_med_rates$med_rate
)

library(RColorBrewer)
mycolor <- brewer.pal(8, "Paired")[c(1,2,5,6)]
pdf("04_all_med_rates_under_sig_TE_ME_boxplot.pdf", width = 4, height = 5)
