...
# Modified By  : Tian Wu
# -------------
# Description  : Process cis-QTL data for SV variants and filter variants with multiple molecular phenotypes
# -------------

suppressPackageStartupMessages(library(tidyverse))
suppressPackageStartupMessages(library(data.table))

setwd("/home/wtian/project/HZAU_cohort_meth/wdy_assist_rerun/09_mediation_analysis/01_cis_QTL_overlap/SV_all_QTL")
...
file_list = file_list[grepl("SV", file_list)]
file_list_molphe = str_replace_all(basename(dirname(dirname(file_list))), "SV-", "")
...
fwrite(data, file = "all_cis_SV_QTL.tsv", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
fwrite(data_flt, file = "all_cis_SV_QTL_at_least_2molphe_including_eQTL.tsv", sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)